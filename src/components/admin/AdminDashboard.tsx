...
const AdminAnalytics = dynamic(() => import('./AdminAnalytics').then(m => ({ default: m.AdminAnalytics })), { ssr: false, loading: () => <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{[...Array(4)].map((_, i) => <div key={i} className="h-64 bg-gray-100 dark:bg-gray-800 rounded animate-pulse" />)}</div> })
